...
import { carDList, carDetailsList, carSelectB, carSelects, carSelectsA } from '../Redux/slice';
import './Compare.scss';
...
const Compare = () => {
  ...
  const SelectCars = useSelector(({ allDetail }) => allDetail.carSelectsA);

  const SelectCars1 = useSelector(({ allDetail }) => allDetail.carSelectB);
    ...
    <div>
      <>
        <div className='container-two'>
          <h2>Compare Two Cars</h2>
          <p>CarWale brings you comparison of
            {[SelectCars]?.map((value, index) => {
              return (
                <span key={index}>
            ...
            {[SelectCars]?.map((value, index) => {
              return (
                <span key={index}>
                  <span>{value.company} {value.model} {value.prize}</span>
                </span>
              )
            })}  and
            {[SelectCars1]?.map((v, i) => {
              return (
                <span key={i}>
                  <span>{v.company} {v.model} {v.prize}</span>
                </span>
              )
            })}.
          </p>
